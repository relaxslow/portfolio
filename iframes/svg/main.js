...
Animate.all = [];
Animate.update = function animationUpdate() {
    let t = Date.now();
    let elapes = t - this.last;
    this.animateAll(elapes)
    this.last = t;

}
Animate.animateAll = function animateAll(delta) {
    if (Animate.all.length == 0) return;
    for (let i = 0; i < Animate.all.length; i++) {
        let [aniFun, host] = Animate.all[i];
...
//start-------------------------------
let scene, sceneUI, camera, cameraUI, controls;
let allObjs = {};
...
function initScene() {
    ...
    let tween = new TWEEN.Tween(triangle.rotation)
        .to({ y: 10 }, 3000)
        .repeat(Infinity)
         .onUpdate(function () {
            renderScene();
