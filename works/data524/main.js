...
function initScene() {
	...
	function loadAllTextures(name) {
		for (let i = 0; i < imgs.length; i++) {
			let texture = new THREE.TextureLoader().load(`/works/data524/headshot/${imgs[i]}.jpg`);
	...
	loadHtml(tipPanel, "/works/data524/detailInfo");
	tipPanel.style.visibility = "hidden";
...
function initCamUI() {
	let camera = new THREE.OrthographicCamera(0, AreaWid, 0, AreaHei, 1, 1000);
...
function render() {
	...
	renderer.clearDepth();
	renderer.setViewport(0, 0, AreaWid, AreaHei);
